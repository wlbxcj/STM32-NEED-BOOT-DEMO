...
#if(OS_MEM_AUTO_EN)
    plink = (void *)(nblks * blksize);
    pmem->OSMemLast     = (void *)((INT32U)addr + (INT32U)plink);
#endif
    *perr               = OS_ERR_NONE;
    return (pmem);
}
...
#if (OS_MEM_AUTO_EN)
void *OSMalloc(int size, INT8U *perr)
{
    OS_MEM * lpMem = (void *)0;
    INT16U   i;
    void * pblk;
    ...
    for (i = 0; i < OS_MAX_MEM_PART; i++)
    {
        if (OSMemTbl[i].OSMemAddr == (void *)0)
            continue;
        if (size <= OSMemTbl[i].OSMemBlkSize)
        {
