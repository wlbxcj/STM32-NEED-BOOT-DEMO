  ...
  else if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    else if (SDIO_BusWide_4b == WideMode)
    {
      ...
      if (SD_OK == errorstatus)
      {
        ...
        SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
        SDIO_Init(&SDIO_InitStructure);
        DBG_Print(3, "SDIO_BusWide_4b is ok\r\n");
      }
    }
    else
    {
...
SD_Error Test_SDCard(void)
{
	...
	Status = SD_Init();   
  	...
  	{		  	
   		...
   		DBG_Print(3, "SD_Init is ok\r\n");
	 	Status = SD_GetCardInfo(&qq);	 	   
  	}								
  	if (Status == SD_OK)
  	{																	 
	  	DBG_Print(3, "SD_GetCardInfo is ok\r\n");
    	Status = SD_SelectDeselect((uint32_t) (qq.RCA << 16));  
  	}
	 
  	if (Status == SD_OK)
  	{
  		DBG_Print(3, "SD_SelectDeselect is ok\r\n");
  	...
  	if (Status == SD_OK)
  	{ 
	    DBG_Print(3, "SD_Erase is ok\r\n"); 
   	    Status = SD_SetDeviceMode(SD_DMA_MODE);	
	    DBG_Print(3, "MicroSD卡读写测试成功\r\n");// 
  	}  
	return Status;
}
